...
export default function Navigation() {
  ...
  // Only redirect if user is authenticated, not when modal opens/closes
  useEffect(() => {
    if (auth.user) {
      handleClose();
    }
  ...
  // Separate effect for path checks
  useEffect(() => {
    // Only navigate away if we're on login/register AND the user is authenticated
          ...
          <div className="fixed inset-0 z-40 flex">
            ...
            >
              <Dialog.Panel className="relative flex w-full max-w-xs flex-col overflow-y-auto bg-white pb-12 shadow-xl">
                ...
                {!auth.user?.firstName && (
                  <div className="space-y-6 border-t border-gray-200 px-4 py-6">
                    <div className="flow-root">
                      <button
                        onClick={() => {
                          setOpen(false); // First close the mobile menu
                          setTimeout(() => handleOpen(), 100); // Then open auth modal with a small delay
                        }}
                ...
                {auth.user?.firstName && (
                  <div className="space-y-6 border-t border-gray-200 px-4 py-6">
                    <div className="flow-root flex items-center gap-2">
                      <Avatar
                        className="text-white"
                        sx={{
                          bgcolor: deepPurple[500],
                    ...
                    <div className="flow-root">
                      <button
                        onClick={() => {
                          setOpen(false);
                          setTimeout(() => navigate('/account/order'), 100);
                        }}
                        className="-m-2 block p-2 font-medium text-gray-900 w-full text-left"
                      >
                        My Orders
                              ...
                              <div className="relative bg-white">
                                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10 py-16">
                                    <div className="col-start-2 grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                                      {category.featured.map((item) => (
                                        <div
                                          key={item.name}
                                          className="group relative text-base sm:text-sm"
                                    ...
                                    <div className="row-start-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-10 text-sm">
                                      {category.sections.map((section) => (
                                        <div key={section.name}>
                                          <p
                                            id={`${section.name}-heading`}
